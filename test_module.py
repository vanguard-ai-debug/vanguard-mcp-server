...
def test_imports():
    ...
    try:
        ...
        print("✓ AegisRAGClient 和 run 函数导入成功")
...
def test_environment():
    ...
    base_url = os.getenv("AEGIS_RAG_BASE_URL")
    api_key = os.getenv("AEGIS_RAG_API_KEY")

    print(f"AEGIS_RAG_BASE_URL: {base_url or '未设置 (将使用默认值)'}")
    print(f"AEGIS_RAG_API_KEY: {'已设置' if api_key else '未设置'}")

    if not base_url:
        print("提示: 可以设置环境变量 AEGIS_RAG_BASE_URL")

    if not api_key:
        print("提示: 建议设置环境变量 AEGIS_RAG_API_KEY")
...
def test_client_creation():
    ...
    try:
        from weknora_mcp_server import AegisRAGClient

        base_url = os.getenv("AEGIS_RAG_BASE_URL", "http://localhost:8080/api/v1")
        api_key = os.getenv("AEGIS_RAG_API_KEY", "test_key")

        client = AegisRAGClient(base_url, api_key)
        print("✓ AegisRAGClient 创建成功")
...
def main():
    """运行所有测试"""
    print("AegisRAG MCP Server 模组测试")
    print("=" * 50)
