...
4. `python aegis-rag_mcp_server.py` - 直接运行
5. `python -m aegis-rag_mcp_server` - 模块运行
6. `aegis-mcp-server` - 安装后命令行工具
7. `aegis-rag-server` - 安装后命令行工具 (别名)
