# AegisRAG MCP Server 可运行模组包 - 项目总结
...
├── 📦 核心文件
│   ├── __init__.py              # 包初始化文件
│   ├── aegis-rag_mcp_server.py   # MCP 服务器核心实现
...
aegis-mcp-server               # 主命令
aegis-rag-server                   # 别名命令
```

### 7. 生产环境安装
```bash
pip install .                    # 生产安装
aegis-mcp-server              # 全局命令
```
...
# 2. 设置环境变量
export AEGIS_RAG_BASE_URL="http://localhost:8080/api/v1"
export AEGIS_RAG_API_KEY="your_api_key"
...
2. **连接错误**: 检查 `AEGIS_RAG_BASE_URL` 设置
3. **认证错误**: 验证 `AEGIS_RAG_API_KEY` 配置
...
**项目状态**: ✅ 完成并可投入使用
**项目仓库**: https://github.com/NannaOlympicBroadcast/AegisRAGMCP
**最后更新**: 2025年10月
**版本**: 1.0.0