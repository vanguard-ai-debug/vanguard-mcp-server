...
AEGIS_RAG_BASE_URL = os.getenv("AEGIS_RAG_BASE_URL", "http://localhost:8080/api/v1")
AEGIS_RAG_API_KEY = os.getenv("AEGIS_RAG_API_KEY", "")


class AegisRAGClient:
    """Client for interacting with AegisRAG API"""

    def __init__(self, base_url: str, api_key: str):
        """Initialize the AegisRAG API client with base URL and authentication"""
    ...
    def _request(self, method: str, endpoint: str, **kwargs) -> Dict[str, Any]:
        """Make a request to the AegisRAG API
...
app = Server("aegis-rag-server")
# Initialize AegisRAG API client with configuration
client = AegisRAGClient(AEGIS_RAG_BASE_URL, AEGIS_RAG_API_KEY)


# AegisRAG 原生工具列表（与 handle_call_tool 中的分支一致）
def _aegis_tools() -> list[types.Tool]:
    return [
        # Tenant Management
        types.Tool(
            name="create_tenant",
            description="Create a new tenant in AegisRAG",
            inputSchema={
                "type": "object",
                "properties": {
...
async def handle_list_tools() -> list[types.Tool]:
    """List all available tools：AegisRAG 原生工具 + 上游 MCP 聚合工具（ext__服务名__工具名）"""
    aegis = _aegis_tools()
    upstream = fetch_upstream_tools()
    return aegis + upstream
...
async def handle_call_tool(
    ...
    try:
        ...
        # 上游 MCP 聚合：ext__服务名__工具名 转发到对应上游
        parsed = parse_prefixed_name(name)
        if parsed is not None:
            service_name, tool_name = parsed
            return call_upstream_tool(service_name, tool_name, args)
